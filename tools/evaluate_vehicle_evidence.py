...
def valid_verified_at(value):
    """Evidence dates are recorded in Japan local time; CI runners operate in UTC."""
    try:
        return date.fromisoformat(value) <= datetime.now(JST).date()
...
def decide_group(vehicle_id, field, items):
    ...
    if len(by_value) > 1:
        return "review", {"vehicle_id": vehicle_id, "field": field, "reason": "conflicting_values", "evidence": items}
    ...
    if official:
        return "accepted", {"vehicle_id": vehicle_id, "field": field, "value": value, "decision": "auto_confirm_official", "confidence": "A", "evidence": same}
    if len(independent_domains) >= 2:
        return "accepted", {"vehicle_id": vehicle_id, "field": field, "value": value, "decision": "auto_confirm_cross_checked", "confidence": "B", "evidence": same}
    return "pending", {"vehicle_id": vehicle_id, "field": field, "value": value, "reason": "insufficient_independent_evidence", "evidence": same}
...
def evaluate_records(records, registry, valid_vehicle_ids=None):
    ...
    for rec in records:
        ...
        if not vehicle_id or not field or "value" not in rec or not url:
            invalid.append({**rec, "invalid_reason": "missing_required_field"}); continue
        if valid_vehicle_ids is not None and vehicle_id not in valid_vehicle_ids:
            invalid.append({**rec, "invalid_reason": "unknown_vehicle_id"}); continue
        if not value_is_sane(field, rec.get("value")):
            invalid.append({**rec, "invalid_reason": "implausible_field_value"}); continue
        if not valid_verified_at(rec.get("verified_at")):
            invalid.append({**rec, "invalid_reason": "invalid_verified_at"}); continue

        domain = domain_of(url)
        detected_type = registry_source_type(url, registry)
        declared_type = rec.get("source_type")
        if not domain:
            invalid.append({**rec, "invalid_reason": "invalid_or_non_https_source_url"}); continue
        if declared_type and declared_type != detected_type:
            invalid.append({**rec, "invalid_reason": "source_type_domain_mismatch", "declared_source_type": declared_type, "detected_source_type": detected_type}); continue

        duplicate_key = (vehicle_id, field, json.dumps(rec.get("value"), ensure_ascii=False, sort_keys=True), url)
        if duplicate_key in seen:
            invalid.append({**rec, "invalid_reason": "duplicate_evidence"}); continue
        seen.add(duplicate_key)

        source_type = detected_type
        weight = policies.get(source_type, {}).get("weight", 0)
        grouped[(vehicle_id, field)].append({**rec, "source_type": source_type, "source_weight": weight, "normalized_value": normalize_value(field, rec.get("value")), "source_domain": domain})

    accepted, review, pending = [], [], []
    for (vehicle_id, field), items in grouped.items():
        bucket, result = decide_group(vehicle_id, field, items)
        if bucket == "accepted": accepted.append(result)
        elif bucket == "review": review.append(result)
...
def main():
    ...
    accepted, review, pending, invalid = evaluate_records(evidence.get("records", []), registry, valid_vehicle_ids=valid_vehicle_ids)
    result = {"schema_version": "1.0.3", "accepted_count": len(accepted), "review_count": len(review), "pending_count": len(pending), "invalid_count": len(invalid), "accepted": accepted, "review": review, "pending": pending, "invalid": invalid}
    OUT.write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    REVIEW.write_text(json.dumps({"schema_version": "1.0.3", "dataset": "vehicle_review_queue", "records": review + pending}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    CHANGE.write_text(json.dumps({"schema_version": "1.0.3", "dataset": "vehicle_change_plan", "production_master_write": False, "records": accepted}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
