(() => {
  ...
  function bindEvents() {
    ...
    [els.wheelMaker, els.wheelBrand, els.wheelPattern, els.wheelSize, els.wheelColor, els.wheelPcd, els.wheelHoles, els.wheelLimit].forEach(input => input.addEventListener("input", renderWheels));
    ...
    [els.wheelMakerChips, els.wheelBrandChips, els.wheelPatternChips, els.wheelSizeChips, els.wheelPcdChips].forEach(container => container.addEventListener("click", handleWheelChipClick));
  ...
  async function importWheelFile(file, wheelType) {
    ...
    try {
      ...
      const data = dedupe(products).map(item => {
        const maker = wheelType === "bs" ? "BS" : item.maker;
        return { ...item, maker, sourceType: wheelType, sourceLabel: label, id: stableId([wheelType, item.productCode, maker, item.patternName, item.sizeText, item.color, item.holes, item.pcd, item.wholesalePrice, item.salePrice]) };
      });
      if (wheelType === "other") {
  ...
  function readWheelRow(row, map, source, sourceRow, context = {}) {
    ...
    const salePrice = roundPrice((wholesalePrice || listPrice || directSale || 0) * state.settings.wheelMarkup);
    return {
      productCode,
      maker,
  ...
  function parseBsUnlistedRows(rows, source) {
    ...
    for (let rowIndex = headerIndex + 2; rowIndex < rows.length; rowIndex++) {
      ...
      out.push({
        ...
        salePrice: roundPrice(price * state.settings.wheelMarkup),
        sourceSheet: source.sheetName,
  ...
  function renderWheels() {
    ...
    const filtered = state.wheelProducts
      ...
      .slice(0, limit);
    renderWheelChips();
    els.wheelSearchSummary.textContent = summaryText([sourceSummary([els.useBsWheel.checked && "BS", els.useOtherWheel.checked && "社外"], "アルミ"), els.wheelMaker.value, els.wheelBrand.value, els.wheelPattern.value, els.wheelSize.value, els.wheelPcd.value && `PCD ${els.wheelPcd.value}`]);
  ...
  function renderWheelChips() {
    ...
    const sizeFiltered = patternFiltered.filter(item => !size || norm(item.sizeText).includes(size));
    ...
    const sizes = [...new Set(patternFiltered.map(item => item.sizeText).filter(Boolean))].sort((a, b) => a.localeCompare(b, "ja")).slice(0, 48);
    const pcds = [...new Set(sizeFiltered.map(item => item.pcd).filter(Boolean))].sort((a, b) => Number(a) - Number(b)).slice(0, 24);
  ...
  function wheelSalePrice(item) {
    const base = number(item?.basePrice || item?.wholesalePrice || item?.dealerCost || item?.salePrice);
    return base > 0 ? roundPrice(base * state.settings.wheelMarkup) : 0;
  }
  ...
  function updateRatePreview() {
    if (els.wheelRatePreview) els.wheelRatePreview.textContent = Number(state.settings.wheelMarkup || 0).toFixed(2);
    if (els.bsRateLabels) {
