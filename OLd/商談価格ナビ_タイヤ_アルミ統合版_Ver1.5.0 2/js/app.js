(() => {
  ...
  const PRIMARY_WHEEL_INCHES = [14, 15, 16, 17, 18, 19, 20];
  ...
  function bindEvents() {
    ...
    [els.taxRate, els.rounding, els.tireAddition, els.defaultRate, els.wheelMarkup, els.wheelPricingMode].forEach(input => input.addEventListener("input", () => {
  ...
  async function importTireFile(file, tireType) {
    ...
    try {
      ...
      saveSourceMeta(sourceKeyForTire(tireType), { status: "loaded", fileName: file.name, count: data.length });
      refreshActiveProducts();
      ...
      saveSourceMeta(sourceKeyForTire(tireType), { status: "error", fileName: file.name, count: 0, message: error.message || "読込エラー" });
    }
  }
  ...
  async function importWheelFile(file, wheelType) {
    ...
    try {
      ...
      saveSourceMeta(sourceKeyForWheel(wheelType), { status: "loaded", fileName: file.name, count: data.length });
      refreshActiveProducts();
      ...
      saveSourceMeta(sourceKeyForWheel(wheelType), { status: "error", fileName: file.name, count: 0, message: error.message || "読込エラー" });
    }
  }
  ...
  function renderTires() {
    ...
    const filtered = state.tireProducts
      ...
      .slice(0, limit);
    renderTireChips();
    els.tireSearchSummary.textContent = summaryText([sourceSummary([els.useSummerTire.checked && "夏", els.useWinterTire.checked && "冬"], "タイヤ"), tireCategoryLabel(tireCategory), brand, els.tireProduct.value, tireInch && `${tireInch}インチ`, els.tireSize.value]);
  ...
  function renderWheels() {
    ...
    const filtered = state.wheelProducts
      ...
      .slice(0, limit);
    renderWheelChips();
    els.wheelSearchSummary.textContent = summaryText([sourceSummary([els.useBsWheel.checked && "BS", els.useOtherWheel.checked && "社外"], "アルミ"), els.wheelMaker.value, els.wheelBrand.value, els.wheelPattern.value, els.wheelInch.value && `${els.wheelInch.value.replace(/インチ/g, "")}インチ`, els.wheelPcd.value && `PCD ${els.wheelPcd.value}`, els.wheelSize.value]);
  ...
  function renderEstimate(focusCostKey = "") {
    ...
    const workTotal = costLines.filter(line => line.group === "work").reduce((sum, line) => sum + line.total, 0);
    const optionTotal = costLines.filter(line => line.group === "option").reduce((sum, line) => sum + line.total, 0);
    const total = tireSingle * 4 + wheelSingle * 4 + workTotal + optionTotal;
    const mismatch = inchMismatch();
    els.estimateItems.innerHTML = `
      <dt>選択中のタイヤ</dt><dd>${state.selectedTire ? escapeHtml(`${state.selectedTire.brand} ${state.selectedTire.subbrand} ${tireDisplaySize(state.selectedTire)}`) : "未選択"}</dd>
  ...
  function renderPrintSheet({ tireSingle, wheelSingle, costLines, total, mismatch }) {
    ...
    const tireTitle = state.selectedTire ? `${state.selectedTire.brand} ${state.selectedTire.subbrand || ""}` : "未選択";
    ...
    const wheelRows = [
      ...
      ["PCD", state.selectedWheel ? wheelFitmentKey(state.selectedWheel) || "—" : "—"],
      ["インセット", state.selectedWheel ? wheelInsetValue(state.selectedWheel) || "—" : "—"],
      ["本数", "4本"],
  ...
  function estimateCostLines(labor = currentLabor()) {
    ...
    return [
      { key: "mount", printKey: "labor", group: "work", label: "組替工賃", qty: 4, unit: labor.mount, defaultTotal: defaults.mount, note: labor.discountAmount ? `通常 ${yen(labor.mountNormalTotal)} / セット割引 ${state.settings.setDiscountRate}%` : "" },
  ...
  function sourceStatusInfo(key, label, data = []) {
    ...
    const statusLabel = status === "loaded" ? "🟢 読込済み" : status === "error" ? "🟠 読込エラー" : "🔴 未読込";
    return {
      label,
      key,
      statusLabel,
      fileName: meta.fileName || (inferredLoaded ? "保存済みデータ" : "—"),
      loadedAt: formatDateTime(meta.loadedAt) || (inferredLoaded ? "保存済み" : "—"),
      count,
