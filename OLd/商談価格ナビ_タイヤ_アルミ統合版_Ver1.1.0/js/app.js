(() => {
  ...
  const state = {
    tireWorkbook: null,
    wheelWorkbook: null,
    tireData: loadJson(TIRE_KEY, []),
    wheelData: loadJson(WHEEL_KEY, []),
  ...
  state.tireProducts = state.tireData;
  state.wheelProducts = state.wheelData;

  const els = {
    saveStatus: $("#saveStatus"),
    tireFile: $("#tireFile"),
    wheelFile: $("#wheelFile"),
    tireFileSetting: $("#tireFileSetting"),
    wheelFileSetting: $("#wheelFileSetting"),
  ...
  function bindEvents() {
    ...
    [els.tireFile, els.tireFileSetting].filter(Boolean).forEach(input => input.addEventListener("change", event => event.target.files[0] && importTireFile(event.target.files[0])));
    [els.wheelFile, els.wheelFileSetting].filter(Boolean).forEach(input => input.addEventListener("change", event => event.target.files[0] && importWheelFile(event.target.files[0])));
  ...
  async function importTireFile(file) {
    ...
    try {
      ...
      state.tireWorkbook = { fileName: file.name, loadedAt: new Date().toISOString(), sheetCount: catalog.sheets?.length || 0 };
      state.tireData = catalog.products
        .filter(item => item.brand && item.size && Number(item.cost) > 0)
        .map(item => ({ ...item, id: stableId([item.brand, item.subbrand, item.size, item.code, item.cost]) }));
      state.tireProducts = state.tireData;
      localStorage.setItem(TIRE_KEY, JSON.stringify(state.tireData));
      setMessage(els.tireStatus, `${file.name}：${state.tireData.length.toLocaleString("ja-JP")}件を読み込みました。アルミ価格表は保持しています。`);
      populateTireBrands();
      renderTireChips();
      renderTires();
      setSaved();
  ...
  async function importWheelFile(file) {
    ...
    try {
      ...
      state.wheelWorkbook = { fileName: file.name, loadedAt: new Date().toISOString(), sheetCount: workbook.SheetNames?.length || 0 };
      const products = [];
      ...
      state.wheelData = dedupe(products).map(item => ({ ...item, id: stableId([item.productCode, item.maker, item.patternName, item.sizeText, item.color, item.holes, item.pcd, item.wholesalePrice, item.salePrice]) }));
      state.wheelProducts = state.wheelData;
      localStorage.setItem(WHEEL_KEY, JSON.stringify(state.wheelData));
      ensureImageMasterFromWheelPatterns();
      setMessage(els.wheelStatus, `${file.name}：${state.wheelData.length.toLocaleString("ja-JP")}件を読み込みました。タイヤ価格表は保持しています。`);
      renderWheelChips();
      renderWheels();
      renderImageManager();
      setSaved();
  ...
  function renderTires() {
    ...
    const filtered = state.tireProducts
      ...
      .slice(0, limit);
    renderTireChips();
    els.tireSearchSummary.textContent = summaryText([brand, els.tireProduct.value, tireInch && `${tireInch}インチ`, els.tireSize.value]);
  ...
  function renderWheels() {
    ...
    const filtered = state.wheelProducts
      ...
      .slice(0, limit);
    renderWheelChips();
    els.wheelSearchSummary.textContent = summaryText([els.wheelMaker.value, els.wheelPattern.value, inch && `${inch}インチ`, els.wheelPcd.value && `PCD ${els.wheelPcd.value}`, els.wheelHoles.value && `${els.wheelHoles.value}穴`, els.wheelSize.value, els.wheelColor.value]);
  ...
  function resetAllData() {
    ...
    [STORE_KEY, IMAGE_KEY, TIRE_KEY, WHEEL_KEY].forEach(key => localStorage.removeItem(key));
    location.reload();
  }
