...
try {
  ...
  const unverifiedModel = await page.evaluate(() => IntegratedApp.state.vehicleSearchRecords.find(item => !item.has_verified_fitment)?.model || 'QA未登録');
  await page.locator('#vehicleModelSearch').fill(unverifiedModel);
  check(await page.locator(`[data-vehicle-filter=model][data-value="${unverifiedModel}"]`).count() === 0, '適合未登録車を商談候補から除外');
